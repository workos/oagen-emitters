...
describe('elixir/tests', () => {
  it('emits test_helper, the fixtures loader, fixtures, and per-service tests', () => {
    const paths = generate().map((f) => f.path);
    expect(paths).toContain('test/test_helper.exs');
    expect(paths).toContain('test/support/test_fixtures.ex');
