...
describe('elixir/resources', () => {
  ...
  it('skips url-builder operations and deduplicates method names', () => {
    const services: Service[] = [
      {
        name: 'SSO',
        operations: [
    ...
    ctx.resolvedOperations = buildResolvedOps(services).map((r) =>
      r.operation.path === '/sso/authorize' ? { ...r, urlBuilder: true } : r,
    );
    const files = generateResources(services, ctx);
    const content = files[0].content;
    expect(content).not.toContain('get_authorization_url');
