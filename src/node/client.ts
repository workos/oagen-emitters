...
function generateWorkOSClient(spec: ApiSpec, ctx: EmitterContext): GeneratedFile {
  ...
  lines.push(
    "import { NoApiKeyProvidedException } from './common/exceptions/no-api-key-provided.exception';",
  );
  ...
  lines.push(
    "      const envKey = typeof process !== 'undefined' ? process.env?.WORKOS_API_KEY : undefined;",
  );
  ...
  lines.push("");
  lines.push(
    "  async get<Result = any>(path: string, options: GetOptions = {}): Promise<{ data: Result }> {",
  );
  ...
  lines.push(
    "      case 422: throw new UnprocessableEntityException({ code, errors, message, requestID });",
  );
  lines.push("      case 429: {");
  lines.push(
  ...
  lines.push(
    "        throw new RateLimitExceededException(message ?? 'Too many requests', requestID, retryAfter);",
  );
  lines.push("      }");
  lines.push(
    "      default: throw new GenericServerException(response.status, message ?? 'Server error', requestID);",
  );
  lines.push("    }");
  lines.push("  }");
...
const RESERVED_BARREL_NAMES = new Set([
  "List",
  "ListResponse",
  "ListMetadata",
  "AutoPaginatable",
...
function generateBarrel(spec: ApiSpec, ctx: EmitterContext): GeneratedFile {
  ...
  for (const enumDef of spec.enums) {
    ...
    lines.push(
      `export type { ${enumDef.name} } from './${dir}/interfaces/${fileName(enumDef.name)}.interface';`,
    );
  }
