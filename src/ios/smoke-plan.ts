...
/**
 * Emit `.oagen-ios-smoke.json` — an authoritative, transform-aware call plan the
 * Swift smoke runner consumes to build driver calls.
 *
 ...
 * context, so every method/service/param/enum-type it lists matches the
 * generated SDK exactly. Split (wrapper) and URL-builder operations are omitted
 * (the runner skips them). Not merged into a live SDK (`integrateTarget: false`).
 */
export function generateSmokePlan(ctx: EmitterContext): GeneratedFile {
  ...
  for (const group of groupByMount(rctx).values()) {
    ...
    for (const resolved of group.resolvedOps) {
      ...
      operations[httpKey] = { service, method: resolveMethodName(op, group.name, rctx), params };
    }
  }

  return {
    path: '.oagen-ios-smoke.json',
...
/**
 * Describe how to serialize a runtime value to a Swift literal for a given IR
 * type. Enum type names are resolved with the emitter's `typeName`, so they
 * match the generated enums (which the smoke runner cannot otherwise compute).
 */
function serializeDescriptor(ref: TypeRef): Serialize {
  ...
  switch (base.kind) {
    ...
    case 'primitive':
      if (base.type === 'unknown') return { kind: 'anycodable' };
      if (base.format === 'date-time' || base.format === 'date') return { kind: 'date' };
      if (base.format === 'binary' || base.format === 'byte') return { kind: 'data' };
      if (base.type === 'string') return { kind: 'string' };
      if (base.type === 'integer') return { kind: 'int' };
    ...
    case 'literal':
      ...
      if (typeof base.value === 'number') return Number.isInteger(base.value) ? { kind: 'int' } : { kind: 'double' };
