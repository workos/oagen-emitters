 ...
 * `.oagen-manifest.json`. Uses the same mount grouping and method-name
 * resolution as `generateResources`, so the smoke runner resolves the exact
 * methods the emitter produced. Split operations map to an array (one entry per
 * wrapper). Reserved-word back-ticks are stripped — the manifest records the
 * logical selector; the smoke generator re-escapes if needed.
 */
